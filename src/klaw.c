...
{
    PORTA->PCR[S3] |= PORT_PCR_IRQC(0xA);  // Przerwanie na zbocze opadające
    PTA->PDDR &= ~S3_MASK;  // **Ustawienie pinu S1 jako wejściowego!**
    NVIC_SetPriority(PORTA_IRQn, 3);       // Priorytet przerwania
    NVIC_ClearPendingIRQ(PORTA_IRQn);
    NVIC_EnableIRQ(PORTA_IRQn);
}
